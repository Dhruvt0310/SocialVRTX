      ...
      <div className="relative px-4 py-8 sm:px-6">
        <div className="absolute left-0 top-0 pointer-events-none select-none">
          <div className="w-[90px] h-[90px] sm:w-[110px] sm:h-[110px]">
            <Image src="/SV/S.svg" alt="S" width={110} height={110} draggable={false} className="w-full h-full" />
          </div>
        </div>

        <div className="absolute right-0 top-0 flex -space-x-5 pointer-events-none select-none">
          {['C', 'I', 'A', 'L'].map((letter) => (
            <div key={letter} className="w-[90px] h-[90px] sm:w-[110px] sm:h-[110px]">
              <Image src={`/SV/${letter}.svg`} alt={letter} width={110} height={110} draggable={false} className="w-full h-full" />
            </div>
          ))}
        </div>

        <div className="absolute left-1/2 top-[58%] -translate-x-1/2 flex -space-x-5 pointer-events-none select-none">
          {['V', 'R', 'T', 'X'].map((letter) => (
            <div key={letter} className="w-[90px] h-[90px] sm:w-[110px] sm:h-[110px]">
              <Image src={`/SV/${letter}.svg`} alt={letter} width={110} height={110} draggable={false} className="w-full h-full" />
            </div>
        ...
        <div className="relative flex justify-center">
          <svg
            ...
            viewBox="0 0 300 600"
            className="h-[40vh] sm:h-[50vh] overflow-visible relative z-10"
            preserveAspectRatio="xMidYMin meet"
          >
            <defs>
              <radialGradient id="bulbGradient1" cx="0.3" cy="0.3" r="0.8">
              ...
              <linearGradient id="capGradient1" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              <linearGradient id="reflectionGradient1" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <motion.g style={{ scale: cordScale, x: cordOffsetX, y: cordOffsetY }}>
              <motion.line
                x1="150"
                x2="150"
                y1="-400"
            ...
            <motion.g style={{ scale: bulbScale, x: bulbOffsetX, y: bulbOffsetY }}>
              <g transform="translate(90,260)">
                <motion.path
                  ...
                  fill="url(#bulbGradient1)"
                  stroke={isOn ? "#000000ff" : "#444"}
                  strokeWidth={isOn ? "5" : "1"}
                  filter={isOn ? "url(#glowFilter1)" : "none"}
                />
                <path
                  d="M 45 75 C 55 70, 65 80, 70 110 C 75 140, 65 160, 55 180"
                  fill="none"
                  stroke="url(#reflectionGradient1)"
                  strokeWidth="5"
                  opacity="0.6"
                  strokeLinecap="round"
                />
                <g transform="translate(-12, -15) scale(1.2)">
                  <path
                    d="M 57 194 L 57 155 M 57 155 C 57 150 58 143 53 141 C 50 140 45 142 46 147 C 48 151 53 152 54 152 C 60 153 65 153 70 152 M 67 194 V 155 C 67 150 66 143 71 141 C 74 140 79 142 78 147 C 76 151 71 152 70 152"
                    fill="none"
                    stroke="#000000ff"
